...
import { PostData, ReviewType } from "@/types/index.type";
...
export default async function BlogPostPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    const { data } = await mealService.getSingleMealById(id);
    return (
        <div>
            <div>
            ...
            <div className="dark:bg-slate-950 text-slate-200 min-h-screen p-4 md:p-10">
                <div className="max-w-6xl mx-auto my-12 p-6 md:p-10 dark:bg-slate-900 rounded-[2.5rem] shadow-[0_20px_50px_rgba(0,0,0,0.3)] border border-slate-800 flex flex-col md:flex-row gap-12 items-center">
                    ...
                    <div className="w-full md:w-1/2 space-y-6">
                        ...
                        <div className="pt-4">
                            <Link href={'/'}>
                                <Button className="h-16 px-10 text-lg font-bold bg-orange-600 hover:bg-orange-500 text-white rounded-2xl shadow-lg shadow-orange-900/20 transition-all active:scale-95 flex gap-3 border-none">
                ...
                <div className="w-full max-w-5xl mx-auto px-4">
                    <Tabs defaultValue="review" className="w-full my-16">
                        ...
                        <TabsContent value="description" className="focus-visible:outline-none">
                            <div className="dark:bg-slate-900 p-8 rounded-2xl border border-slate-800 shadow-2xl min-h-[200px] leading-relaxed text-black dark:text-slate-400">
                        ...
                        <TabsContent value="review" className="focus-visible:outline-none">
                            {/* Post Review Text Box Section */}
                            <div className="dark:bg-slate-900 bg-slate-400 p-5 md:p-8 rounded-[2rem] border border-slate-800 mb-10 shadow-xl">
                                <h3 className="text-xl font-bold text-white mb-6 flex items-center gap-2">
                                    Share Your Feedback
                                </h3>

                                <div className="space-y-5">
                                    {/* Star Rating Selector (Visual Only) */}
                                    <div className="flex items-center gap-4 dark:bg-slate-950/50 w-fit px-4 py-2 rounded-xl border border-slate-800">
                                        <span className="text-sm font-medium dark:text-slate-500">Rating:</span>
                                        <div className="flex items-center gap-1 text-amber-500">
                                            {[...Array(5)].map((_, i) => (
                                                <button key={i} className="hover:scale-110 transition-transform">
                                                    <Star size={20} fill={i < 4 ? "currentColor" : "none"} strokeWidth={2} />
                                                </button>
                                            ))}
                                        </div>
                                    </div>
                                    ...
                                    <div className="relative group">
                                        <textarea
                                            ...
                                            className="w-full bg-slate-950 border border-slate-800 rounded-2xl p-4 md:p-5 text-slate-200 placeholder:text-slate-600 focus:outline-none focus:ring-2 focus:ring-orange-500/20 focus:border-orange-500/40 transition-all resize-none shadow-inner text-base md:text-lg"
                                        />
                                    </div>

                                    {/* Submit Button */}
                                    <div className="flex justify-end">
                                        <Button className="w-full md:w-auto h-12 md:h-14 px-10 text-base font-black bg-orange-600 hover:bg-orange-500 text-white rounded-xl shadow-lg shadow-orange-900/20 transition-all active:scale-95">
                                            POST REVIEW
                                        </Button>
                                    </div>
                                </div>
                            ...
                            <div className="grid gap-4">
                                {data?.reviews?.length > 0 ? (
                                    data.reviews.map((review: ReviewType) => (
                                        <div
                                            key={review.id}
                                            className="group flex flex-col md:flex-row md:items-start gap-5 bg-slate-300 dark:bg-slate-900 p-6 rounded-2xl border border-slate-800 hover:border-slate-700 transition-all shadow-sm"
                                        >
                                            <div className="flex-shrink-0">
                                                <div className="w-14 h-14 rounded-full bg-gradient-to-br from-orange-500 to-rose-600 flex items-center justify-center text-white text-xl font-bold">
