...
import { Users, Utensils, ShoppingBag, DollarSign, Activity, CheckCircle, AlertCircle } from "lucide-react";
import { 
    BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer, 
    AreaChart, Area 
} from 'recharts';

export default function AdminStat({ response }: { response: any }) {
    ...
    const chartData = [
        { name: 'Sat', revenue: stats.orders.totalRevenue * 0.2, orders: stats.orders.total * 0.3 },
        { name: 'Sun', revenue: stats.orders.totalRevenue * 0.5, orders: stats.orders.total * 0.6 },
        { name: 'Mon', revenue: stats.orders.totalRevenue * 0.4, orders: stats.orders.total * 0.4 },
        { name: 'Tue', revenue: stats.orders.totalRevenue * 0.8, orders: stats.orders.total * 0.7 },
        { name: 'Wed', revenue: stats.orders.totalRevenue, orders: stats.orders.total },
    ];
    ...
    return (
        <div className="bg-slate-950 min-h-screen p-4 md:p-10 text-slate-200">
            <div className="max-w-7xl mx-auto">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-10">
                    ...
                    <div className="bg-slate-900/50 border border-slate-800 p-6 rounded-[2.5rem]">
                        <h3 className="text-lg font-bold mb-6 flex items-center gap-2 italic">
                            <DollarSign className="text-emerald-400" size={18} /> Revenue Flow
                        </h3>
                    ...
                    <div className="bg-slate-900/50 border border-slate-800 p-6 rounded-[2.5rem]">
                        <h3 className="text-lg font-bold mb-6 flex items-center gap-2 italic">
                            <ShoppingBag className="text-rose-400" size={18} /> Order Volume
                        </h3>
                        <div className="h-[250px] w-full">
                            <ResponsiveContainer width="100%" height="100%">
                ...
                {/* 3. User Traffic & Stock (Old Layout) */}
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    <section className="bg-slate-900/40 border border-slate-800/60 p-8 rounded-[2.5rem] backdrop-blur-md">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div className="p-4 rounded-2xl bg-slate-950 border border-slate-800/50 text-center">
