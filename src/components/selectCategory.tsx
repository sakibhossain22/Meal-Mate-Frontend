        ...
        <div className="space-y-8">
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4 p-4 rounded-2xl border border-slate-100 shadow-sm mb-8">
                ...
                <div className="flex items-center gap-3 w-full  sm:w-auto">
                    <span className="text-sm font-bold dark:text-slate-300 text-black    uppercase tracking-wider hidden md:inline">Filter By:</span>
                    <Select onValueChange={handleCategoryChange} defaultValue={queryCategory}>
                        <SelectTrigger className="w-full sm:w-52 h-11 rounded-xl border-slate-200 bg-white font-semibold text-slate-700 shadow-sm focus:ring-[#f22e3e] focus:border-[#f22e3e] transition-all">
                            <SelectValue placeholder="Select a Category" />
                        </SelectTrigger>
                        <SelectContent className="rounded-xl border-slate-100 shadow-xl">
                            <SelectGroup>
                                <SelectLabel className="text-slate-400 font-bold">Meal Categories</SelectLabel>
                                <SelectItem value="all" className="cursor-pointer font-medium focus:bg-slate-50">
                                    All Categories
                                </SelectItem>
                                {categories?.map((cat) => (
                                    <SelectItem key={cat.id} value={cat.name} className="cursor-pointer font-medium focus:bg-slate-50">
                                        {cat.name}
                                    </SelectItem>
                                ))}
                            </SelectGroup>
                        ...
                        {meals?.length > 0 ? meals.map((meal: any) => (
                            <div key={meal.id} className="group overflow-hidden rounded-2xl border transition bg-gray-200 hover:shadow-xl dark:bg-slate-900">
                                ...
                                <div className="p-4">
                                    ...
                                    <p>{meal.description.slice(0, 35) + "..."}</p>
